...
local mainMod = "MOD3" -- Super -> F18 (bincur rewire) -> Hyper_L on Mod3 (xkb). ALT freed for window-move binds

hl.bind(mainMod .. " + F", hl.dsp.exec_cmd(terminal))
hl.bind("SUPER + F", hl.dsp.exec_cmd(terminal)) -- rescue: bincur 죽으면 rewire 안 돼서 LeftMeta가 Super 그대로 들어옴
...
-- Switch workspaces with mainMod + [0-9], move active window with ALT + [0-9]
for i = 1, 10 do
    local key = i % 10
...
hl.bind("CTRL + left",  hl.dsp.window.resize({ x = -30, y = 0,   relative = true }), { repeating = true })
hl.bind("CTRL + right", hl.dsp.window.resize({ x = 30,  y = 0,   relative = true }), { repeating = true })
hl.bind("CTRL + up",    hl.dsp.window.resize({ x = 0,   y = -30, relative = true }), { repeating = true })
hl.bind("CTRL + down",  hl.dsp.window.resize({ x = 0,   y = 30,  relative = true }), { repeating = true })

-- Screenshot
hl.bind("Print", hl.dsp.exec_cmd([[/bin/bash -c 'FILE=~/Pictures/$(date +%Y%m%d_%H%M%S).png && grim "$FILE" && wl-copy < "$FILE"']]))
hl.bind(mainMod .. " + Print", hl.dsp.exec_cmd("/bin/bash /home/smh/.local/bin/screenshot-area.sh"))
